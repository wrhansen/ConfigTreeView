...
in the index_map of a ConfigTreeView. Example:

index_map( mapping a column hierachy to an index in the TreeView row):
{'address': {'markup': 6},
 'cell-bg-index': 10,
 'comments': {'markup': 7},
 'contact': {'markup': 3},
...
{
	'market': {'pixbuf': 'images/ebay/ebaylogo.png'},
...
class DataFormatter():
	...
	def get_new_row(self):
		'''
		Returns an empty list with the correct length a TreeModel with the given index_map
		expects.
		'''
	...
	def get_rows(self, data ):
		...
		NOTE: This version of the function uses the treeZip function created by
		ninjagecko from StackOverflow.
		'''
		for row in data:
			new_row = self.get_new_row()
